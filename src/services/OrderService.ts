...
interface PricedOrderItem extends CreateOrderItem {
  itemName: string;
  basePrice: number;
  unitPrice: number;
  subtotal: number;
...
const canMoveToStatus = (order: Orders, nextStatus: string) => {
  ...
  const expected =
    ...
    current === "ready" ? (order.orderType === "pickup" ? "completed" : "in_delivery") :
    current === "in_delivery" ? "completed" : null;
  return expected === nextStatus;
};

const buildModifierSnapshots = (menu: Menus, requested: CreateOrderModifier[] = []) => {
  ...
  for (const choiceId of requestedByChoice.keys()) {
    if (!knownChoiceIds.has(choiceId)) throw new HttpError(400, `La opción ${choiceId} no pertenece a ${menu.itemName || "este producto"}`);
  }
  ...
  for (const group of groups) {
    ...
    if (effectiveSelected.length < min) {
      throw new HttpError(400, `${group.title}: selecciona al menos ${min} opción${min === 1 ? "" : "es"}`);
    }
    if (effectiveSelected.length > max) {
      throw new HttpError(400, `${group.title}: puedes seleccionar máximo ${max} opción${max === 1 ? "" : "es"}`);
    }
    ...
    // Sólo registramos removals explícitos de opciones que venían incluidas por defecto.
    for (const choice of choices) {
      if (choice.isDefault && requestedByChoice.get(choice.choiceId) === "removed") {
        snapshots.push({
          choiceId: choice.choiceId,
          groupTitle: group.title,
          choiceName: choice.name,
...
export class OrderService {
  ...
  async list() {
    const orders = await this.orderRepo.find({
      relations: ["user", "business", "orderDetails", "orderDetails.menu", "orderDetails.orderDetailOptions"],
  ...
  async getById(orderId: number) {
    const order = await this.orderRepo.findOne({
      where: { orderId },
      relations: ["user", "business", "orderDetails", "orderDetails.menu", "orderDetails.orderDetailOptions", "orderStatusHistories"],
    });
  ...
  async getByUser(userId: number) {
    const orders = await this.orderRepo.find({ where: { userId }, relations: DETAIL_RELATIONS, order: { createdAt: "DESC" } });
    return orders.map(formatOrder);
  }
  ...
  async getByBusiness(businessId: number) {
    const orders = await this.orderRepo.find({
      where: { businessId },
      relations: ["user", "orderDetails", "orderDetails.menu", "orderDetails.orderDetailOptions", "orderStatusHistories"],
      order: { createdAt: "DESC" },
    });
    return orders.map(formatOrder);
  }
  ...
  async create(input: CreateOrderInput) {
    const {
      userId, businessId, items, orderType = "pickup", customerName, customerPhone,
      deliveryAddress, deliveryAddressId, deliveryLocation, notes,
    } = input;
    ...
    const newOrderId = await AppDataSource.transaction(async (manager) => {
      ...
      const detailOptionRepo = manager.getRepository(OrderDetailOptions);
      ...
      for (const item of pricedItems) {
        ...
        if (item.modifierSnapshots.length) {
          await detailOptionRepo.save(item.modifierSnapshots.map((modifier) => detailOptionRepo.create({
            orderDetailId: detail.orderDetailId,
            optionId: null,
            choiceId: modifier.choiceId,
            groupTitle: modifier.groupTitle,
            choiceName: modifier.choiceName,
            priceExtra: modifier.priceExtra.toFixed(2),
            selectionState: modifier.selectionState,
          })));
        }
      }
    ...
    const fullOrder = await this.orderRepo.findOne({
      ...
      relations: ["orderDetails", "orderDetails.menu", "orderDetails.orderDetailOptions", "orderStatusHistories"],
    });
  ...
  async updateStatus(orderId: number, status: string, note: string | undefined, actor: OrderActor = {}) {
    ...
    if (order.userId) emitOrderStatusUpdate(order.userId, {
      orderId: order.orderId,
      status: order.status,
