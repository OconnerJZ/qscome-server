...
export class BusinessService {
  ...
  async getMenu(businessId: number) { const menus = await this.menuRepo.find({ where: { businessId, isAvailable: true, isArchived: false }, order: { category: "ASC", itemName: "ASC" } }); return menus.map(formatMenuItem); }
  ...
  async create(input: CreateBusinessInput) {
    ...
    const businessId = await AppDataSource.transaction(async (manager) => {
      ...
      if (schedule?.length) await scheduleRepo.save(schedule.map((s) => scheduleRepo.create({ businessId: business.businessId, ...s })));
      ...
      // El primer negocio convierte la capacidad global del usuario a owner para
      // las rutas administrativas actuales. Sigue conservando todas las funciones
      // de cliente en React (pedir comida, carrito y mis órdenes).
  ...
  async updateSchedules(businessId: number, schedules: any[] = []) { await this.scheduleRepo.delete({ businessId }); if (schedules.length) await this.scheduleRepo.save(schedules.map((s) => this.scheduleRepo.create({ businessId, ...s }))); return this.getById(businessId); }
  async updateDeliverySettings(businessId: number, body: any) { let row = await this.bDeliveryRepo.findOne({ where: { businessId } }); if (!row) row = this.bDeliveryRepo.create({ businessId }); const map: any = { delivery_radius_km: "deliveryRadiusKm", delivery_fee: "deliveryFee", min_order_amount: "minOrderAmount", estimated_time_min: "estimatedTimeMin", use_own_delivery: "useOwnDelivery" }; Object.entries(map).forEach(([from,to]) => { if (body[from] !== undefined) (row as any)[to] = body[from]; }); await this.bDeliveryRepo.save(row); return this.getById(businessId); }
