...
const SESSION_RELATIONS = ["participants", "participants.user", "items", "items.participant", "items.participant.user", "items.menu", "items.menu.business", "items.menu.business.businessPaymentMethods", "suborders", "suborders.order"];
...
export class SharedOrderService {
  ...
  private format(session: SharedOrderSession, self: SharedOrderParticipant) {
    ...
    return { id: session.sessionId, title: session.title || "Orden compartida", status: session.status, version: session.version, expiresAt: session.expiresAt, isHost: self.role === "host", self: { role: self.role, label: sharedParticipantLabel(self.packagingNumber), name: self.user?.userName || "Participante" }, participants: session.participants.filter((participant) => participant.status === "active").map((participant) => ({ label: sharedParticipantLabel(participant.packagingNumber), name: participant.user?.userName || "Participante", role: participant.role, itemCount: session.items.filter((item) => item.participantId === participant.participantId).reduce((sum, item) => sum + item.quantity, 0) })), items, businesses, grandTotal: items.reduce((sum, item) => sum + item.subtotal, 0), suborders: (session.suborders || []).map((suborder) => ({ businessId: suborder.businessId, orderId: suborder.orderId, status: suborder.order?.status })) };
  }
}
