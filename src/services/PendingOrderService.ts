...
export interface PendingOrderModifierInput {
  choiceId: number;
...
export interface PendingOrderItemInput {
  ...
  quantity: number;
  note?: string | null;
  modifiers?: PendingOrderModifierInput[];
}

interface PendingOrderActor {
  userId?: number;
  role?: string;
}
...
interface PricedItem extends PendingOrderItemInput {
  itemName: string;
  unitPrice: number;
  subtotal: number;
  ...
  for (const group of groups) {
    ...
    for (const choice of choices) {
      if (choice.isDefault && requestedByChoice.get(choice.choiceId) === "removed") {
        snapshots.push({ choiceId: choice.choiceId, groupTitle: group.title, choiceName: choice.name, priceExtra: 0, selectionState: "removed" });
      }
    }
  }
...
export class PendingOrderService {
  async replaceItems(orderId: number, items: PendingOrderItemInput[], expectedVersion: number, actor: PendingOrderActor = {}) {
    ...
    await AppDataSource.transaction(async (manager) => {
      ...
      for (const item of pricedItems) {
        ...
        if (item.modifierSnapshots.length) {
          await optionRepo.save(item.modifierSnapshots.map((modifier) => optionRepo.create({
    ...
    const refreshed = await AppDataSource.getRepository(Orders).findOne({
      where: { orderId },
