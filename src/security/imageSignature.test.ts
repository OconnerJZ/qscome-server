...
test("valida la firma real y no sólo el MIME declarado", () => {
  ...
  assert.equal(hasValidImageSignature(Uint8Array.from([0x47, 0x49, 0x46, 0x38, 0x39, 0x61]), "image/gif"), true);
  assert.equal(hasValidImageSignature(Uint8Array.from([0x47, 0x49, 0x46, 0x38, 0x38, 0x61]), "image/gif"), false);
});
