...
export class AuthController {
  ...
  async googleAuth(req: Request, res: Response) {
    try {
      ...
      if (!payload?.email) return res.status(401).json({ success: false, message: "Token inválido" });
      ...
      return res.status(500).json({ success: false, message: "Error en autenticación con Google", error: process.env.NODE_ENV === "development" ? error.message : undefined });
    }
  }

  async facebookAuth(req: Request, res: Response) {
    return res.status(501).json({ success: false, message: "Facebook OAuth aún no implementado" });
  }
